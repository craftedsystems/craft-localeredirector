...
class LocaleRedirectorService extends BaseApplicationComponent
{
  ...
  public function getBrowserLanguageMatch()
  {
    ...
    if ($browserLanguages)
    {
      ...
      // if (craft()->userSession->getUser()->admin) // real admins omly
      if (craft()->userSession->getUser()) // any front-end user
      {
        // Use admin settings if front-end user session exists
        $activatedLocalesSet = craft()->globals->getSetByHandle('localizations')->localizationsAdmin;
      } 
      else 
      {
        // Use public settings
  ...
  private function newUrl($locale)
  {
    ...
    if ($element) {
      ...
      return UrlHelper::getSiteUrl($path, null, null, $locale) . $qs;
    } else {
      return false;
    }
  }
